...
namespace WorldWind.Net.Monitor
{
	...
	public class ProgressMonitor : System.Windows.Forms.Form
	{
        ...
        private long Sta_num_tiles = 0;
        private double Sta_time_mean_per_tile = 0.0;
        private long Sta_total_bytes = 0;
        private double Sta_tiles_second = 0.0;
        private DateTime ori;
        private double Sta_time_total = 0.0;
        private TimeSpan total_monitor_runtime = new TimeSpan();
        private bool Sta_running = false;
		...
		public void Update(WebDownload wd)
		{
            ...
            {
                Utility.Log.Write(Log.Levels.Debug, "NET", "ProgressMonitor -> Update(): Exception: " + ex.Message);
            }
            catch (Exception ex)
            {
                Utility.Log.Write(Log.Levels.Debug, "NET", "DebugItem -> Update(): Exception: " + ex.Message);
            }

		}
        protected void Set_Statistics()
        {
            foreach (DebugItem item in listView.Items)
            {
                if (item.statistics == false && item.Status.Contains("Complete"))
                {
                    item.statistics = true;
                    if (Sta_running == false)
                    {
                        Sta_running = true;
                        ori = DateTime.Now - item.ElapsedTime;
                    }
                    Sta_num_tiles++;
                    Sta_time_total += item.ElapsedTime.TotalSeconds;
                    Sta_time_mean_per_tile = Sta_num_tiles / Sta_time_total;
                    Sta_total_bytes += item.BytesProcessed;
                    total_monitor_runtime = DateTime.Now.Subtract(ori);
                    Sta_tiles_second = Sta_num_tiles / total_monitor_runtime.TotalSeconds;

                    this.toolStripStatusLabel1.Text = "NUM TILES DOWNLOADED = " + Sta_num_tiles + "    MEAN (TILES/SECOND) =" + Sta_time_mean_per_tile.ToString("F3") + " t/s   (NUM TILES)/SECOND =" + Sta_tiles_second.ToString("F2") + " t/s  TOTAL BYTES =" + FormatLength(Sta_total_bytes, true);
                }
            }
        }
		...
		private void InitializeComponent()
		{
            ...
            this.listView.Size = new System.Drawing.Size(768, 152);
            ...
            this.toolStripStatusLabel1.Text = "Statistics:";
            // 
            // ProgressMonitor
            // 
	...
	class DebugItem : ListViewItem
	{
        ...
        // flag to mark if the item has been used in statistics
		...
		public void Update( WebDownload wd )
		{
            ...
            try
            {
                ...
                else if(wd.num_retry != 0) Status = "Retry = " + wd.num_retry;
                else
                    // In progress
                    Status = "Receiving.";
