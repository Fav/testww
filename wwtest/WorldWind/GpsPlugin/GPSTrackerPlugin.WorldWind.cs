...
namespace GpsTracker
{
	...
	{
		public static string m_sVersion = "V04R06";
        public static string m_sOverlayVersion = "(" + m_sVersion + ")";
        ...
        public void pluginAddGeoFence(GPSGeoFenceData geoFence, GPSRenderInformation renderInformation)
        {
            gpsOverlay.AddGeoFence(geoFence, renderInformation);
        }
        ...
        public void pluginSetFileLineNumber(int iIndex, int iLineNumber)
        {
            gpsTracker.SetFileLineNumber(iIndex, iLineNumber);
        }

        public void pluginSetFilePlayPause(int iIndex, bool bPause)
        {
            gpsTracker.SetFilePlayPause(iIndex, bPause);
        }
        ...
        public void pluginToggleShowCircles(int iIndex, bool bShowCircles)
        {
            gpsTracker.SetShowCircles(iIndex, bShowCircles);
        }

        public void pluginToggleShowGrid(int iIndex, bool bShowGrid)
        {
            gpsTracker.SetShowGrid(iIndex, bShowGrid);
        }
	...
	public class GPSTrackerOverlay : RenderableObjectList
	{
        ...
        public GPSTrackerOverlay(GpsTrackerPlugin plugin)
		...
		public override bool PerformSelectionAction(DrawArgs drawArgs)
		{
            ...
            if ((pbKeyState[VK_RALT] & 0x80) == 0x80 ) //TrackLine) //GeoFence
            {
                lock ("RenderAccess")
                {
                    ...
                    if (m_gpsGeoFence[m_iGpsGeoFenceIndex]==null)
                    {
                        ...
                        World.Settings.VerticalExaggeration = 0;

                        GPSRenderInformation renderInfo = new GPSRenderInformation();
                        renderInfo.iPositionUnit = Plugin.gpsTracker.m_iPositionUnit;
                        renderInfo.iDistanceUnit = Plugin.gpsTracker.m_iDistanceUnit;
                        string sName;
                        if (bGeoFence)
                    ...
                    {
                        World.Settings.VerticalExaggeration = m_fCurrentVerticalExaggeration;

                        GPSGeoFenceData geoFence = new GPSGeoFenceData();
                        geoFence.bGeoFence = bGeoFence;
                        geoFence.colorLine = Color.Red;
        ...
        public void AddGeoFence(GPSGeoFenceData geoFence, GPSRenderInformation renderInformation)
        {
            lock ("RenderAccess")
            {
                //Resize array if necessary
		...
		public void RemoveAllOverlay()
		{
            lock ("RenderAccess")
            {
                ...
                for (int i = 0; i < m_uGeoFenceResize; i++)
                {
        ...
        {
            Plugin.pluginSetFileLineNumber(iIndex, iLineNumber);
        }

        public void SetFilePlayPause(int iIndex, bool bPause)
        {
            Plugin.pluginSetFilePlayPause(iIndex, bPause);
        }
        ...
        public void ToggleShowCircles(int iIndex, bool bShowCircles)
        {
            Plugin.pluginToggleShowCircles(iIndex, bShowCircles);
        }
        public void ToggleShowGrid(int iIndex, bool bShowGrid)
        {
            Plugin.pluginToggleShowGrid(iIndex, bShowGrid);
        }

        public void ConvertAltitudeUnits(float dAltitude, int iUnitFrom, int iUnitTo, out float dConvertedAltitude, out string sUnit)
        {
            dConvertedAltitude = dAltitude;
            sUnit = "meters";


            switch (iUnitFrom)
                {
                    case (int)AltitudeUnits.unitMeters:
                        switch (iUnitTo)
                        {
                            case 0: //Metric
                                sUnit = "meters";
                            ...
                            case 1: //Imperial
                            case 2: //Nautical
                                dConvertedAltitude *= 3.28083F;
                                sUnit = "feet";
                                break;
                        }
                        break;
                    case (int)AltitudeUnits.unitFeet:
                        switch (iUnitTo)
                        {
                            case 0: //Metric
                                dConvertedAltitude /= 3.28083F;
        ...
        public void ConvertSpeedUnits(float dSpeed, int iUnitFrom, int iUnitTo, out float dConvertedSpeed, out string sUnit)
        {
            dConvertedSpeed = dSpeed;
            sUnit = "Km/h";

            switch (iUnitFrom)
                {
                    case (int)SpeedUnits.unitKmH:
                        switch (iUnitTo)
                        {
                            case 0: //Metric
                                sUnit = "Km/h";
                                break;
                            case 1: //Imperial
                                dConvertedSpeed *= 0.621371F;
                                sUnit = "Miles/h";
                                break;
                            case 2: //Nautical
                                dConvertedSpeed *= 0.539956F;
                                sUnit = "Knots";
                                break;
                        }
                        break;
                    case (int)SpeedUnits.unitMH:
                        switch (iUnitTo)
                        {
                            case 0: //Metric
                                dConvertedSpeed *= 1.609344F;
                                sUnit = "Km/h";
                                break;
                            case 1: //Imperial
                                sUnit = "Miles/h";
                                break;
                            case 2: //Nautical
                                dConvertedSpeed *= 0.868976F;
                                sUnit = "Knots";
                                break;
                        }
                        break;
                    case (int)SpeedUnits.unitKnots:
                        switch (iUnitTo)
                        {
                            case 0: //Metric
                                dConvertedSpeed *= 1.852F;
                                sUnit = "Km/h";
                                break;
                            case 1: //Imperial
                                dConvertedSpeed *= 1.150779F;
                                sUnit = "Miles/h";
                                break;
                            case 2: //Nautical
                                sUnit = "Knots";
	...
	public class GPSTrackLine : RenderableObject
	{
		...
		private CustomVertex.PositionColored[][] vertices;// = new CustomVertex.PositionColored[][];
		Vector3 v;
		Color m_colorTrack;
		float m_fCurrentVerticalExaggeration;
        ...
        public GPSTrackLine(World parentWorld, string sDescription, GPSRenderInformation renderInfo)
        ...
        public void SetTrack(GPSTrackerOverlay gpsTrackerOverlay, GPSTrack gpsTrack, string sDescription, Color colorTrack, string sIcon, bool bShowInfo, bool bTrackOnTop)
		{
			m_gpsTrack=gpsTrack;
			m_gpsTrackerOverlay = gpsTrackerOverlay;
            m_bShowInfo = bShowInfo;
            m_bTrackOnTop = bTrackOnTop;
        ...
        public override bool PerformSelectionAction(DrawArgs drawArgs)
        {
...
#endif
            {
                    ...
                    Math.Abs(DrawArgs.LastMousePosition.Y - projectedPoint.Y) < m_iIconHeightHalf)
                {
                    ...
                    {
                        if (m_gpsSetup==null)
                            m_gpsSetup = new GpsSetup(null, this, null);
                        m_gpsSetup.Show();
                    }
        ...
        public void ApplyUpdate(bool bDispose, bool bUpdate)
        {
            if (m_gpsSetup!=null)
            {
                m_gpsSetup.Dispose();
		...
		public override void Render(DrawArgs drawArgs)
		{
			...
			if(this.isInitialized)
			{
				...
				if (m_fCurrentVerticalExaggeration != World.Settings.VerticalExaggeration)
				{
                    ...
                    this.vertices = new CustomVertex.PositionColored[m_uVerticesCount][];
					...
					for (int i=0; i<m_uVerticesCount; i++)
					{
						...
						for(uint ii = 0; ii < uLength; ii++) 
						{
							...
							v=MathEngine.SphericalToCartesian(m_gpsTrack.m_fLat[uCount], m_gpsTrack.m_fLon[uCount], this.m_parentWorld.EquatorialRadius + ( (m_gpsTrack.m_fAlt[uCount]+fOffset) * World.Settings.VerticalExaggeration));		
                    ...
                    if (m_bCalculateDistance == true)
                    {
                        ...
                        if (m_RenderInfo.iDistanceUnit == 0)
                        {
                            m_sDistanceUnit = " Km";
                        }
                        else
                        if (m_RenderInfo.iDistanceUnit == 1)
                        {
                            m_sDistanceUnit = " Miles";
                            m_fTotalDistance *= 0.621371F;
                        }
                        else
                        if (m_RenderInfo.iDistanceUnit == 2)
                        {
                            m_sDistanceUnit = " Nautical Miles";
                            m_fTotalDistance *= 0.539956F;
                        }
                    }
				...
				drawArgs.device.RenderState.ZBufferEnable = !m_bTrackOnTop;

                //drawArgs.device.VertexFormat = VertexFormats.Diffuse | VertexFormats.Transformed |  VertexFormats.Diffuse | CustomVertex.PositionColored.Format;
                ...
                //for (int i = 0; i < m_gpsTrack.m_fLat.Length; i++)
                //    DrawPoint(m_gpsTrack.m_fLat[i], m_gpsTrack.m_fLon[i], m_gpsTrack.m_fAlt[i], 0, m_colorTrack.ToArgb(), drawArgs);
                ...
                {
                    string sInfo="";
                    if (m_RenderInfo.bShowName)
                        sInfo += "Track: " + m_sDescription + "\n";

                    if (m_RenderInfo.bShowPosition)
                    {
                        if (m_RenderInfo.iPositionUnit == 1)
                        {
                            string sNS;
                        ...
                        {
                            sInfo += "Lat: " + Convert.ToString(m_fStartLatitud) + "\n"; ;
                            sInfo += "Lon: " + Convert.ToString(m_fStartLongitude) + "\n"; ;
                        }
                    }
                    if (m_RenderInfo.bShowTrackDistance)
                    {
                        sInfo += "Length: " + m_fTotalDistance + m_sDistanceUnit;
                    }

                    //Draw some black shadow around the yellow text
                    int color = Color.Black.ToArgb();
                    if (m_RenderInfo.iInformationFontSize == 0)
                    {
                    ...
                    drawArgs.toolbarFont.DrawText(null, sInfo, (int)projectedPoint.X + (m_iIconWidthHalf) + 11, (int)(projectedPoint.Y) + 1, color);
                        color = m_RenderInfo.colorInformation.ToArgb();
                    drawArgs.toolbarFont.DrawText(null, sInfo, (int)projectedPoint.X + (m_iIconWidthHalf) + 10, (int)(projectedPoint.Y), color);
                }
                    else
                    if (m_RenderInfo.iInformationFontSize == 1)
                    {
                        drawArgs.defaultSubTitleDrawingFont.DrawText(null, sInfo, (int)projectedPoint.X + (m_iIconWidthHalf) + 11, (int)(projectedPoint.Y), color);
                        drawArgs.defaultSubTitleDrawingFont.DrawText(null, sInfo, (int)projectedPoint.X + (m_iIconWidthHalf) + 11, (int)(projectedPoint.Y) + 1, color);
                        color = m_RenderInfo.colorInformation.ToArgb();
                        drawArgs.defaultSubTitleDrawingFont.DrawText(null, sInfo, (int)projectedPoint.X + (m_iIconWidthHalf) + 10, (int)(projectedPoint.Y), color);
                    }
                }
	...
	public class GPSIcon : RenderableObject
	{
        ...
        CustomVertex.PositionColored[][] verticesCircle = new CustomVertex.PositionColored[100][];
        CustomVertex.PositionColored[][] verticesCircleLines = new CustomVertex.PositionColored[100][];
        CustomVertex.PositionColored[][] verticesGridLines = new CustomVertex.PositionColored[100*100*4][];
		float m_fVerticalExaggeration;

		public bool m_bTrack;
        public bool m_bTrackHeading;

        public GpsSetup m_gpsSetup;
        public PlaybackControl m_gpsPlaybackControl;
        public bool m_bPlay;
			...
			: base(renderInformation.sDescription, parentWorld.Position, Quaternion.RotationYawPitchRoll(0,0,0)) 
		{
            ...
            m_bTrackOnTop = m_RenderInfo.fTrackOnTop;
		...
		public override void Dispose()
		{
            ...
            if (m_gpsPlaybackControl != null)
                m_gpsPlaybackControl.Dispose();
			...
			#endif
			{
					...
					Math.Abs(DrawArgs.LastMousePosition.Y-projectedPoint.Y)<m_iIconHeightHalf)
				{
					...
					if ((pbKeyState[VK_LCONTROL] & 0x80)==0x80) //Toggle Information text
					{
                        if (m_gpsSetup == null)
                        {
                            m_gpsSetup = new GpsSetup(this, null, null);
                            m_gpsSetup.Show();
                        }
                        else
                            m_gpsSetup.WindowState = FormWindowState.Normal;
					}
                    else
                    if ((pbKeyState[VK_RCONTROL] & 0x80) == 0x80 && m_RenderInfo.iTotalLines>0) //play back control
                    {
                        if (m_gpsPlaybackControl == null)
                        {
                            m_gpsPlaybackControl = new PlaybackControl(this, m_RenderInfo.sDescription, m_RenderInfo.iTotalLines, m_RenderInfo.iCurrentLine);
                            m_gpsPlaybackControl.Show();
                        }
                        else
                            m_gpsPlaybackControl.WindowState = FormWindowState.Normal;
                    }
					else
					{
						bool bOn=true;
        ...
        public void ApplyUpdate(bool bDispose, bool bUpdate)
        {
            ...
            if (bUpdate)
            {
                ...
                m_gpsTrackerOverlay.ToggleShowCircles(m_RenderInfo.iIndex, m_RenderInfo.bShowCircles);
                m_gpsTrackerOverlay.ToggleShowGrid(m_RenderInfo.iIndex, m_RenderInfo.bShowGrid);
        ...
        public void PlaybackUpdatePosition(int iLineNumber)
        {
            m_gpsTrackerOverlay.SetFileLineNumber(m_RenderInfo.iIndex, iLineNumber);
        }

        public void PlaybackPlayPause(bool bPause)
        {
            m_gpsTrackerOverlay.SetFilePlayPause(m_RenderInfo.iIndex, bPause);
            m_bPlay = !bPause;
        }
		...
		public override void Render(DrawArgs drawArgs)
		{
			...
			if (m_RenderInfo.bPOI)
			{
                ...
                xyzPosition = MathEngine.SphericalToCartesian(this.m_RenderInfo.fLat, this.m_RenderInfo.fLon, this.m_parentWorld.EquatorialRadius + (elevation * World.Settings.VerticalExaggeration));
			}
			else
			{
            ...
            if (m_gpsPlaybackControl != null && m_bPlay)
                m_gpsPlaybackControl.UpdatePosition(m_RenderInfo.iCurrentLine);
        ...
        void ShowInfo(DrawArgs drawArgs, Vector3 projectedPoint, float fDistance)
		{
            ...
            if (m_RenderInfo.bShowName)
			    sInfo += m_RenderInfo.sDescription + "\n";
            ...
            if (m_RenderInfo.bShowPosition)
            {
                ...
                {
                    sInfo += "Lat: " + Convert.ToString(m_RenderInfo.fLat) + "\n"; ;
                    sInfo += "Lon: " + Convert.ToString(m_RenderInfo.fLon) + "\n"; ;
                }
			...
			if (m_RenderInfo.bPOI==false)
			{
				...
				if (m_RenderInfo.fAlt==-1000000F)
				{
                    ...
                    {
						sInfo += "Altitude: " + Convert.ToString(m_fLastAlt) + " " + m_RenderInfo.sAltUnit + " (Last Known)\n";
				}
				}
				else
				{
                    float fAltitude;
                    m_gpsTrackerOverlay.ConvertAltitudeUnits(m_RenderInfo.fAlt, m_RenderInfo.iAltitudeUnit, m_RenderInfo.iDistanceUnit, out fAltitude, out m_RenderInfo.sAltUnit);
                    sInfo += "Altitude: " + Convert.ToString(fAltitude) + " " + m_RenderInfo.sAltUnit + "\n";
                    m_fLastAlt = fAltitude;
				}
				...
				if (m_RenderInfo.fESpeed==-1000000F && m_fLastESpeed==-1000000F)
				{
					...
					{
                        float fSpeed;
                        m_gpsTrackerOverlay.ConvertSpeedUnits(m_RenderInfo.fSpeed, m_RenderInfo.iSpeedUnit, m_RenderInfo.iDistanceUnit, out fSpeed, out m_RenderInfo.sSpeedUnit);
                        sInfo += "Speed: " + Convert.ToString(fSpeed) + " " + m_RenderInfo.sSpeedUnit + "\n";
                        m_fLastSpeed = fSpeed;
					}
				}
                }

                if (m_RenderInfo.bShowHeading)
                {
				...
				if (m_fTotalDistance>=0F)
				{
                    ...
                    if (m_RenderInfo.iDistanceUnit == 0)
                    {
                        sUnit = (m_fTotalDistance >= 1F) ? " Km" : " Meters";
                    ...
                    {
                        sUnit = " Miles";
                        fDistance = m_fTotalDistance * 0.621371F;
                    }
                    else
                    if (m_RenderInfo.iDistanceUnit == 2)
                    {
                        sUnit = " Nautical Miles";
                        fDistance = m_fTotalDistance * 0.539956F;
                    }
                    else
                    {
                        sUnit = (m_fTotalDistance >= 1F) ? "km" : "m";
                        fDistance = (m_fTotalDistance < 1F) ? (m_fTotalDistance * 1000F) : m_fTotalDistance;
                    }
				...
				if (m_RenderInfo.sComment!="" && m_RenderInfo.bShowComment)
					sInfo += "Comment: " + m_RenderInfo.sComment + "\n";

                if (m_RenderInfo.bShowComment)
				sInfo += m_RenderInfo.sPortInfo;
			}
			//Draw some black shadow around the yellow text
            if (m_RenderInfo.iInformationFontSize == 0 && sInfo!="")
            {
			drawArgs.toolbarFont.DrawText(null, sInfo, (int)projectedPoint.X + (m_iIconWidthHalf) +11, (int)(projectedPoint.Y),color );
			drawArgs.toolbarFont.DrawText(null, sInfo, (int)projectedPoint.X + (m_iIconWidthHalf) +11, (int)(projectedPoint.Y)+1,color );
                color = m_RenderInfo.colorInformation.ToArgb();
            ...
            if (m_RenderInfo.iInformationFontSize == 1 && sInfo != "")
            {
                drawArgs.defaultSubTitleDrawingFont.DrawText(null, sInfo, (int)projectedPoint.X + (m_iIconWidthHalf) + 11, (int)(projectedPoint.Y), color);
                drawArgs.defaultSubTitleDrawingFont.DrawText(null, sInfo, (int)projectedPoint.X + (m_iIconWidthHalf) + 11, (int)(projectedPoint.Y) + 1, color);
                color = m_RenderInfo.colorInformation.ToArgb();
		...
		public void SetGpsData(DrawArgs drawArgs, GPSRenderInformation renderInformation)
		{
			...
			if (m_fVerticalExaggeration != World.Settings.VerticalExaggeration)
			{
				...
				for (uint uVertices=0; uVertices<=m_uVerticesCount; uVertices++)
				{
					for (uint uP=0; uP<uPoint; uP++)
					{
						...
						Vector3 v=MathEngine.SphericalToCartesian(this.vertices[uVertices].fLat[uP], this.vertices[uVertices].fLon[uP], this.m_parentWorld.EquatorialRadius + ((this.vertices[uVertices].fAlt[uP]+fOffset) * World.Settings.VerticalExaggeration));
			...
			Vector3 vTrack=MathEngine.SphericalToCartesian(renderInformation.fLat, renderInformation.fLon, this.m_parentWorld.EquatorialRadius + ((renderInformation.fAlt+fOffsetT) * World.Settings.VerticalExaggeration));
    ...
    public class GPSGeoFence : RenderableObject
    {
        ...
        Vector3 xyzPosition;
        float m_fTotalDistance = 0F;
        bool m_bCalculateDistance = true;
        string m_sDistanceUnit = "";
        ...
        GPSRenderInformation m_RenderInfo;
        ...
        public GPSGeoFence(World parentWorld, string sDescription, GpsTrackerPlugin Plugin, GPSRenderInformation renderInformation, bool bGeoFence, Color colorTrack)
            : base(sDescription, parentWorld.Position, Quaternion.RotationYawPitchRoll(0, 0, 0))
        {
            m_parentWorld = parentWorld;
            m_Plugin = Plugin;
            ...
            m_bTrackOnTop = m_Plugin.gpsTracker.m_bTrackOnTop;
        }
...
#endif
            {
                this.texture = TextureLoader.FromFile(drawArgs.device, GpsTrackerPlugin.m_sPluginDirectory + "\\GeoFenceVertice.png", 0, 0, 1, 0, Format.Unknown, Pool.Managed, Filter.Box, Filter.Box, 0);
            }
#if !DEBUG
			catch(Microsoft.DirectX.Direct3D.InvalidDataException)
			{
			...
			try
#endif
                {
                        ...
                        Math.Abs(DrawArgs.LastMousePosition.Y - projectedPoint.Y) < m_iIconHeightHalf)
                    {
                        ...
                        {
                            if (m_gpsSetup==null)
                                m_gpsSetup = new GpsSetup(null, null, this);
                            m_gpsSetup.Show();
                            bRet = true;
                        }
                    }
        ...
        public override void Render(DrawArgs drawArgs)
        {
            ...
            if (m_bGeoFence == false && m_bCalculateDistance == true && arrayLat.Count >= 2)
            {

                //Calculate distance;
                m_fTotalDistance = 0;
                for (int iList = 0; iList < arrayLat.Count-1; iList++)
                {


                            double dDistance, dLatTo, dLatFrom, dLonFrom, dLonTo, deltaLon, deltaLat;
                            dLatTo = (float)arrayLat[iList+1];
                            dLatFrom = (float)arrayLat[iList];
                            dLonTo = (float)arrayLon[iList+1];
                            dLonFrom = (float)arrayLon[iList];
            ...
            for (int iList = 0; iList < arrayLat.Count; iList++)
            {
                ...
                if (drawArgs.WorldCamera.ViewFrustum.ContainsPoint(xyzPosition))
                {
                    ...
                    if (iList == 0 && m_bShowInfo && m_RenderInfo.bShowName)
                    {
                        string sInfo = "";
                        if (m_bGeoFence)
                            sInfo = "GeoFence:\r\n" + m_sDescription + "\r\n";
                        ...
                        {
                            sInfo = "Track Line:\r\n" + m_sDescription + "\r\n";
                            sInfo += "Length: " + Convert.ToString(m_fTotalDistance) + m_sDistanceUnit;
                        }
                        //Draw some black shadow around the yellow text
                        ...
                        if (m_RenderInfo.iInformationFontSize == 1)
                        {
                            drawArgs.defaultSubTitleDrawingFont.DrawText(null, sInfo, (int)projectedPoint.X + (m_iIconWidthHalf) + 1, (int)(projectedPoint.Y), color);
                            drawArgs.defaultSubTitleDrawingFont.DrawText(null, sInfo, (int)projectedPoint.X + (m_iIconWidthHalf) + 1, (int)(projectedPoint.Y) + 1, color);
                            color = m_RenderInfo.colorInformation.ToArgb();
                            drawArgs.defaultSubTitleDrawingFont.DrawText(null, sInfo, (int)projectedPoint.X + (m_iIconWidthHalf) + 0, (int)(projectedPoint.Y), color);
                        }
                    }
                }
                ...
                if (iList > 0)
                {
                    ...
                    if (m_bDone && iList == arrayLat.Count - 1 && m_bGeoFence)
                    {
